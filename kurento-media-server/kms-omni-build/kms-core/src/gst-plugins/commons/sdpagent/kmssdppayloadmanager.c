...
G_DEFINE_TYPE_WITH_PRIVATE (KmsSdpPayloadManager,
...
kms_sdp_payload_manager_class_init (KmsSdpPayloadManagerClass * klass)
{
  ...
  g_type_interface_add_prerequisite (KMS_TYPE_SDP_PAYLOAD_MANAGER, KMS_TYPE_I_SDP_PAYLOAD_MANAGER);
      g_type_add_interface_static (KMS_TYPE_SDP_PAYLOAD_MANAGER, KMS_TYPE_I_SDP_PAYLOAD_MANAGER,
                                   &(const GInterfaceInfo) {
                                       (GInterfaceInitFunc) kms_i_sdp_playload_manager_iface_init,
                                       NULL,
                                       NULL
                                   });
}
...
//  self->priv = KMS_SDP_PAYLOAD_MANAGER_GET_PRIVATE (self);
  self->priv = kms_sdp_payload_manager_get_instance_private (self);
