...
G_DEFINE_TYPE_WITH_PRIVATE (KmsSdpBaseGroup, kms_sdp_base_group,
                                G_TYPE_OBJECT)
...
kms_sdp_base_group_class_init (KmsSdpBaseGroupClass * klass)
{
  ...
  // g_type_class_add_private (klass, sizeof (KmsSdpBaseGroupPrivate));
    ...
    g_type_interface_add_prerequisite (KMS_TYPE_SDP_BASE_GROUP, KMS_TYPE_I_SDP_SESSION_EXTENSION);
    g_type_add_interface_static (KMS_TYPE_SDP_BASE_GROUP, KMS_TYPE_I_SDP_SESSION_EXTENSION,
                                 &(const GInterfaceInfo) {
                                     (GInterfaceInitFunc) kms_i_sdp_session_extension_init,
                                     NULL,
                                     NULL
                                 });
}
...
//  self->priv = KMS_SDP_BASE_GROUP_GET_PRIVATE (self);
  self->priv = kms_sdp_base_group_get_instance_private (self);
  self->priv->id = -1;
}
